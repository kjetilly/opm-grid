...
long long FaceQuadrature::numQuadPts() const
{
...
void FaceQuadrature::quadPtCoord(const long long index, double* coord) const
{
    ...
    // Degree 2 case.
    const long long nn = grid_.face_nodepos[face_ + 1] - grid_.face_nodepos[face_];
    const long long* fnodes = grid_.face_nodes + grid_.face_nodepos[face_];
    ...
    if (index < nn) {
        ...
        const long long node0 = fnodes[index];
        const long long node1 = fnodes[(index + 1)%nn];
        ...
        const long long node = fnodes[index - nn];
        for (long long dd = 0; dd < dim; ++dd) {
...
double FaceQuadrature::quadPtWeight(const long long index) const
{
    if (degree_ < 2) {
    ...
    if (index < nn) {
        ...
        // Interiour edge midpoint.
        // Recall that index is now in [nn, 2*nn).
        const long long node0 = fnodes[(index - 1) % nn];
        const long long node1 = fnodes[index - nn];
        const long long node2 = fnodes[(index + 1) % nn];
