...
namespace Opm
{
    ...
    SparseTable<long long> cellNeighboursAcrossVertices(const UnstructuredGrid& grid)
    {
        ...
        const long long num_vertices = grid.number_of_nodes;
        std::vector<std::set<long long>> v2c(num_vertices);
        const long long num_faces = numFaces(grid);
        ...
        for (long long face = 0; face < num_faces; ++face) {
            for (unsigned nodepos = grid.face_nodepos[face]; nodepos < grid.face_nodepos[face + 1]; ++nodepos) {
                const long long vertex = grid.face_nodes[nodepos];
                for (long long face_nb = 0; face_nb < 2; ++face_nb) {
                    const long long face_nb_cell = fc(face, face_nb);
                    if (face_nb_cell >= 0) {
        ...
        SparseTable<long long> cell_nb;
        const long long num_cells = numCells(grid);
        ...
        for (long long cell = 0; cell < num_cells; ++cell) {
            ...
            const long long num_cell_faces = cell_faces.size();
            for (long long local_face = 0; local_face < num_cell_faces; ++local_face) {
                const long long face = cell_faces[local_face];
                for (unsigned nodepos = grid.face_nodepos[face]; nodepos < grid.face_nodepos[face + 1]; ++nodepos) {
                    const long long vertex = grid.face_nodes[nodepos];
                               ...
                               SparseTable<long long>& nb)
    {
        ...
        typedef std::pair<double, long long> AngleAndPos;
        std::vector<AngleAndPos> angle_and_pos;
        std::vector<long long> original;
        for (long long cell = 0; cell < num_cells; ++cell) {
            const long long num_nb = nb[cell].size();
            angle_and_pos.clear();
            angle_and_pos.resize(num_nb);
            for (long long ii = 0; ii < num_nb; ++ii) {
                const long long cell2 = nb[cell][ii];
