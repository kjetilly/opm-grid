...
long long CellQuadrature::numQuadPts() const
{
    ...
    for (unsigned hf = grid_.cell_facepos[cell_]; hf < grid_.cell_facepos[cell_ + 1]; ++hf) {
        const long long face = grid_.cell_faces[hf];
        sumnodes += grid_.face_nodepos[face + 1] - grid_.face_nodepos[face];
    }
    return 4*sumnodes;
}

void CellQuadrature::quadPtCoord(const long long index, double* coord) const
{
    ...
    if (dim == 2) {
        if (index % 3 == 0) {
            ...
            const long long hface = grid_.cell_facepos[cell_] + index/3;
            ...
            const long long hface = grid_.cell_facepos[cell_] + index/3;
            const long long face = grid_.cell_faces[hface];
            const long long nodeoff = (index % 3) - 1; // == 0 or 1
            const long long node = grid_.face_nodes[grid_.face_nodepos[face] + nodeoff];
    ...
    assert(dim == 3);
    long long tetindex = index / 4;
    const long long subindex = index % 4;
    const double* nc = grid_.node_coordinates;
    for (unsigned hf = grid_.cell_facepos[cell_]; hf < grid_.cell_facepos[cell_ + 1]; ++hf) {
        const long long face = grid_.cell_faces[hf];
        const long long nfn = grid_.face_nodepos[face + 1] - grid_.face_nodepos[face];
        if (nfn <= tetindex) {
        ...
        const double* fc = grid_.face_centroids + dim*face;
        const long long* fnodes = grid_.face_nodes + grid_.face_nodepos[face];
        const long long node0 = fnodes[tetindex];
        const long long node1 = fnodes[(tetindex + 1) % nfn];
...
double CellQuadrature::quadPtWeight(const long long index) const
{
    if (degree_ < 2) {
    ...
    if (dim == 2) {
        const long long hface = grid_.cell_facepos[cell_] + index/3;
        const long long face = grid_.cell_faces[hface];
        const long long* nptr = grid_.face_nodes + grid_.face_nodepos[face];
